...
export function init(dbPath = DEFAULT_DB_PATH) {
  ...
  try {
    ...
    try {
      db.exec(`
        CREATE TABLE IF NOT EXISTS cover_media (
        ...
        CREATE INDEX IF NOT EXISTS idx_cover_media_media ON cover_media(media_id);
      `);
    } catch (err) {
      // table creation is best-effort; errors surface later via API if schema missing
    }
